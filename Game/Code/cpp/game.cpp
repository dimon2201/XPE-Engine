...
void GameApp::WindowClosed()
{
    LogWarning("GameApp::WindowClosed()");
}

void GameApp::KeyPressed(const eKey key)
{
    if (key == eKey::Esc)
    {
        WindowManager::Close();
...
void GameApp::KeyHold(const eKey key)
{
    MoveLight(key);
}

void GameApp::CursorMoved(const double x, const double y)
{
...
void GameApp::MoveLight(const eKey key)
{
    if (key == eKey::Up) {
