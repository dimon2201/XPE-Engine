...
#define hot_alloc(size) xpe::core::MMemory::HotPools->Allocate(size)
#define hot_allocT(T, count) static_cast<T*>(hot_alloc(sizeof(T) * count))
#define hot_construct(T) xpe::core::MMemory::HotPools->AllocateConstruct<T>()
#define hot_construct_args(T, ...) xpe::core::MMemory::HotPools->AllocateConstructArgs<T>(__VA_ARGS__)
#define hot_free(addr) xpe::core::MMemory::HotPools->Free(addr)
#define hot_destruct(T, addr) \
addr->~T();                \
hot_free(addr)

#define main_alloc(size) xpe::core::MMemory::MainPools->Allocate(size)
#define main_allocT(T, count) static_cast<T*>(main_alloc(sizeof(T) * count))
#define main_construct(T) xpe::core::MMemory::MainPools->AllocateConstruct<T>()
#define main_construct_args(T, ...) xpe::core::MMemory::MainPools->AllocateConstructArgs<T>(__VA_ARGS__)
#define main_free(addr) xpe::core::MMemory::MainPools->Free(addr)
...
namespace xpe {
    ...
    namespace core {
        ...
        ENGINE_API void* align16_alloc(usize size);
        ...
        bool operator==(const cHotAllocator<T>&, const cHotAllocator<U>&)
        {
            return true;
        }
        ...
        template<class T, class U>
        bool operator==(const cMainAllocator<T>&, const cMainAllocator<U>&)
        {
            return true;
        }
