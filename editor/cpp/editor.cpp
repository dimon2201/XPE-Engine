...
namespace focus {
    ...
    void Editor::Init()
    {
        ...
        Input::WindowClosedEvents->AddEvent(this, OnWindowClosed<Editor>, 1);
        Input::KeyPressedEvents->AddEvent(this, OnKeyPressed<Editor>, 1);
        Input::KeyHoldEvents->AddEvent(this, OnKeyHold<Editor>, 1);
        Input::CursorMovedEvents->AddEvent(this, OnCursorMoved<Editor>, 1);
...
//                    MaterialManager::AddAOFromFile(*material, "resources/materials/steel/ao.png");
    ...
    void Editor::WindowClosed()
    {
        LogWarning("GameApp::WindowClosed()");
    }

    void Editor::KeyPressed(const eKey key)
    {
        if (key == eKey::Esc)
        {
            WindowManager::Close();
    ...
    void Editor::KeyHold(const eKey key)
    {
        MoveLight(key);
    }

    void Editor::CursorMoved(const double x, const double y)
    {
    ...
    void Editor::MoveLight(const eKey key)
    {
        if (key == eKey::Up) {
