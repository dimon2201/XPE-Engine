    ...
    namespace res {
        ...
        static void SetKeyFrames(sBone& bone, const aiNodeAnim* channel)
        {
            for (int i = 0; i < channel->mNumPositionKeys; i++)
            {
                ...
                bone.KeyPositions.emplace_back(timestamp, cAssimpManager::ToVec3(aiPosition));
            }
            ...
            for (int i = 0; i < channel->mNumRotationKeys; i++)
            {
                ...
                bone.KeyRotations.emplace_back(timestamp, cAssimpManager::ToQuat(aiOrienation));
            }
            ...
            for (int i = 0; i < channel->mNumScalingKeys; i++)
            {
                ...
                bone.KeyScales.emplace_back(timestamp, cAssimpManager::ToVec3(aiScale));
            }
        }
        ...
        static void ParseSkeletFromMesh(aiMesh* mesh, sSkeleton& skelet)
        {
            ...
            for (int i = 0; i < mesh->mNumBones; i++)
            {
                ...
                if (bones.find(boneName) == bones.end())
                {
                    ...
                    newBone.Offset = cAssimpManager::ToMat4(mesh->mBones[i]->mOffsetMatrix);
